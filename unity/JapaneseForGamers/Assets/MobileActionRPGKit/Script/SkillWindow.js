...
function AssignSkill(id : int , sk : int){
	...
	player.GetComponent(AttackTrigger).skill[id].delay = dataSkill.skill[skillListSlot[sk]].delay;
	print(sk);
...
function AssignAllSkill(){
	...
	while(n <= 2){
		...
		player.GetComponent(AttackTrigger).skill[n].delay = dataSkill.skill[skill[n]].delay;
		//-------Assign Icon to Skill Button-----------
		if(skillButton[n]){

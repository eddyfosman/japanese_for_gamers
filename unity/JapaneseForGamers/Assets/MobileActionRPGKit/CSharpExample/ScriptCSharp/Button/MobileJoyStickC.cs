...
public class MobileJoyStickC : MonoBehaviour {
	...
	void Start() {
		...
		Vector2 originalInset = new Vector2(GetComponent<GUITexture>().pixelInset.x, GetComponent<GUITexture>().pixelInset.y);

		Rect insetRect = GetComponent<GUITexture>().pixelInset;
		insetRect.x = originalInset.x;
		insetRect.y = originalInset.y;

		//Reset GUI Pixel Inset to original position
		GetComponent<GUITexture>().pixelInset = insetRect;

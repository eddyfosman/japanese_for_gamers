	...
	public void GetMercenary(int id) {
		if (player.GetComponent<InventoryC>().cash >= mercenariesInfo[id].price){
			...
			if (player.GetComponent<SpawnPartnerC>()) {
				...
				if(player.GetComponent<SpawnPartnerC>().currentPartner){
					Destroy(player.GetComponent<SpawnPartnerC>().currentPartner);
				}
			} else {
			...
			if (mercenariesInfo[id].mercenaryPrefab) {
				//Spawn Mercenary
				GameObject m = (GameObject)Instantiate(mercenariesInfo[id].mercenaryPrefab , player.transform.position , player.transform.rotation);
				m.GetComponent<AIfriendC>().master = player.transform;
				player.GetComponent<SpawnPartnerC>().currentPartner = m;
